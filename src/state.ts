...
export default async function getState (rawCookie: string): Promise<State> {
  ...
  const userId = extract(Number, req.data, /<meta name=user-data data-userid=(\d+)/, 'Unable to determine current user id')
  const csrfToken = extract(String, req.data, /Roblox.XsrfToken.setToken\('(.+)'\)/, 'Unable to extract CSRF token')
